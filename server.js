...
const server = http.createServer(
    ...
    (req, res) => {
        ...
        // --- NTFY PROXY ENDPOINT ---
        if (req.url === '/api/notifications/send' && req.method === 'POST') {
            readJsonBody(req)
                .then(async (payload) => {
                    const { title, message, tags, topic } = payload;
                    if (!message) {
                        sendJson(res, 400, { error: 'Message missing' });
                        return;
                    }
                    // Use built-in fetch (Node 18+)
                    try {
                        const ntfyResp = await fetch(`https://ntfy.sh/${topic || '33min-vfx-stream-8k2p'}`, {
                            method: 'POST',
                            body: message,
                            headers: {
                                'Title': title || 'Alert',
                        ...
                        if (ntfyResp.ok) {
                            ...
                            sendJson(res, 502, { error: 'Ntfy upstream error', details: await ntfyResp.text() });
                        }
                    } catch (err) {
                        sendJson(res, 500, { error: 'Internal Server Error', details: err.message });
                    }
                })
                .catch((e) => sendJson(res, 400, { error: 'Invalid JSON', details: e.message }));
            return;
        }
